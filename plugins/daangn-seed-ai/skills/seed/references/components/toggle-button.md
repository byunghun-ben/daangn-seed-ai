...
ToggleButton 은 YAML 스펙 상 4 개의 슬롯 (root / label / prefixIcon / suffixIcon / progressCircle) 을 가지지만, **React API 는 flat 단일 컴포넌트** `<ToggleButton>` 1 개다. 아이콘과 progressCircle 슬롯은 CSS className (`seed-prefix-icon`, `seed-suffix-icon`) 으로 스타일만 주입되고, 실제 DOM 은 children 에 넣는 `PrefixIcon` / `SuffixIcon` 컴포넌트가 담당한다.
...
| `progressCircle` | ⚪ | `loading=true` 일 때 자동 — 직접 렌더하지 않음 | loading spinner. 자세한 내용은 [States](#states) 의 loading 섹션 참조 |
...
- `loading` prop 은 progressCircle 을 **자동으로** 렌더한다 — 사용자가 직접 `<ProgressCircle>` 을 children 에 넣지 않는다.
...
| `loading` (미선택 + 로딩) | `loading` prop = `true`, `pressed`=`false` | 버튼은 pressed 배경으로 변하고 progressCircle 자동 렌더 | `$color.bg.brand-solid-pressed` |
| `selected,loading` | `loading`=`true`, `pressed`=`true` | 선택 상태 배경을 pressed 변형으로 표시 + progressCircle | `$color.bg.neutral-weak-pressed` |
...
### loading 동작 — progressCircle 자동 렌더
...
- CSS recipe (`toggle-button.css`) 의 `.seed-toggle-button[data-loading]` 셀렉터가 이 attribute 에 반응해 **progressCircle slot 을 pseudo-element 로 표시** (또는 업스트림 구현에서는 CSS-driven spinner).
- 배경색 역시 `[data-loading]` 규칙에 의해 `-pressed` 변형으로 전환 (위 표의 `loading` / `selected,loading` 행 참조).
- **따라서 개발자는 `<ProgressCircle>` 컴포넌트를 children 에 직접 넣지 않는다** — `loading` prop 하나만으로 자동 처리된다.
...
| slot `progressCircle` | `loading={true}` 자동 렌더 | 직접 렌더 금지 |
...
ㄨ <ToggleButton pressed={on} onPressedChange={setOn}>
     <IconBellFill />
     알림
   </ToggleButton>
...
// loading 중 progressCircle 을 직접 렌더
ㄨ <ToggleButton loading>
     <ProgressCircle />
     저장 중
   </ToggleButton>
   // ProgressCircle 이 중복 렌더됨 (loading prop 이 자동 주입).

// loading 만 걸고 disabled 생략
ㄨ <ToggleButton loading={saving} pressed={on} onPressedChange={setOn}>
     토글
   </ToggleButton>
...
ㄨ <ToggleButton><PrefixIcon svg={<IconBellFill/>} /></ToggleButton>
   // AT 사용자는 무엇을 토글하는지 알 수 없음.

// size 리터럴 아닌 값
ㄨ <ToggleButton size="medium">토글</ToggleButton>
...
ㄨ <ToggleButton onPressedChange={({ pressed }) => setPressed(pressed)}>
     토글
   </ToggleButton>
...
`loading={true}` 한 번으로 progressCircle slot 이 **자동 렌더**되고 배경색도 `-pressed` 변형으로 전환된다. 직접 `<ProgressCircle>` 을 children 에 넣지 않는다.
